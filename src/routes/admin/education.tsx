...
function EducationPage() {
  ...
  const handleRestore = async (id: string) => {
    await restoreEducation({ data: { id } });
    router.invalidate();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      await editEducation({ data: { id: editingId, ...formData } });
    } else {
      await addEducation({ data: formData });
    }
  ...
  return (
    <div className="space-y-6 relative">
      ...
      <div className="flex gap-4 border-b border-gray-200 dark:border-zinc-800">
        ...
        >
          Active Education ({active.length})
        </button>
        <button 
            ...
            {items.length === 0 ? (
              <tr>
                <td colSpan={3} className="p-8 text-center text-gray-500">
                  {activeTab === 'active' ? 'No active education found.' : 'Recycle bin is empty.'}
                </td>
              </tr>
            ) : items.map((item) => (
              <tr key={item.id} className="border-b border-gray-200 dark:border-zinc-800 last:border-0 hover:bg-gray-50 dark:hover:bg-zinc-800/50">
                <td className="p-4">
                  ...
                  {item.description && <p className="text-sm mt-1 line-clamp-1">{item.description}</p>}
                </td>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-xl max-w-lg w-full max-h-[90vh] overflow-y-auto border border-gray-200 dark:border-zinc-800">
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              <div>
                ...
                <input required type="text" value={formData.degree} onChange={e => setFormData({...formData, degree: e.target.value})} className="w-full p-2 rounded border border-gray-300 dark:border-zinc-700 bg-transparent" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Institution</label>
                <input required type="text" value={formData.institution} onChange={e => setFormData({...formData, institution: e.target.value})} className="w-full p-2 rounded border border-gray-300 dark:border-zinc-700 bg-transparent" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Duration</label>
                <input required type="text" value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} placeholder="e.g. 2018 - 2022" className="w-full p-2 rounded border border-gray-300 dark:border-zinc-700 bg-transparent" />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Description (Optional)</label>
                <textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} className="w-full p-2 rounded border border-gray-300 dark:border-zinc-700 bg-transparent min-h-[100px]" />
              </div>
